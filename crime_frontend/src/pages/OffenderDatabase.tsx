              ...
              <div className="bg-slate-800/60 rounded-lg p-3 space-y-2 text-xs">
                {[
                  ...
                  <div key={String(label)} className="flex justify-between">
                    ...
                    <span className="text-slate-200 font-medium">{val == null ? "—" : String(val)}</span>
                  </div>
                ))}
              </div>
                ...
                <div className="bg-slate-800/60 rounded-lg p-3 space-y-1">
                  ...
                  {Object.entries(modusOperandi).filter(([k]) => k !== 'is_fallback').map(([k, v]) => (
                    <div key={k} className="flex justify-between text-xs">
                      ...
                      <div className="text-slate-200 text-right max-w-[60%]"><AIMarkdown text={Array.isArray(v) ? v.join(", ") : (typeof v === 'object' && v !== null ? JSON.stringify(v) : String(v ?? ""))} isFallback={k === 'ai_mo_summary' ? (modusOperandi.is_fallback as boolean) : false} /></div>
                    </div>
                  ))}
                </div>
              )}
